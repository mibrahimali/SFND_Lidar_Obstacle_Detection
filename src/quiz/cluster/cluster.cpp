...
pcl::visualization::PCLVisualizer::Ptr initScene(Box window, int zoom)
{
	...
	viewer->setBackgroundColor (0, 0, 0);
  	...
  	viewer->addCube(window.x_min, window.x_max, window.y_min, window.y_max, 0, -.001, 1, 1, 1, "window");
  	return viewer;
}
...
void buildCluster(std::vector<int> &cluster,const int pointId, const std::vector<std::vector<float>>& points,std::vector<bool> &pointsProcessed, KdTree* tree, float distanceTol)
{
	pointsProcessed[pointId] = true;
	cluster.push_back(pointId);
	std::vector<int> nearby = tree->search(points[pointId],distanceTol);
  	for(int index : nearby)
	  if (!pointsProcessed[index])
		{
			buildCluster(cluster,index,points,pointsProcessed, tree, distanceTol);
		}
...
std::vector<std::vector<int>> euclideanCluster(const std::vector<std::vector<float>>& points, KdTree* tree, float distanceTol)
{
	...
	for (int pointId=0;pointId <points.size();++pointId)
	{
		if (!pointsProcessed[pointId])
		{
			...
			buildCluster(cluster,pointId,points,pointsProcessed, tree, distanceTol);
